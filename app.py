            ...
            with col1:
                ...
                if audio_file is not None:
                    ...
                    if st.button('Record'):
                        ...
                        wav, sr = librosa.load("grabacion.wav", sr=44100)
                        Xdb = get_melspec("grabacion.wav")[1]
                        mfccs = librosa.feature.mfcc(wav, sr=sr)
                        # display audio
                        st.audio("grabacion.wav", format='audio/wav', start_time=0)
                        path = "grabacion.wav"
                        audio_file = "test"
            ...
            if model_type == "mfccs":
                ...
                with st.container():
                    ...
                    with col2:
                        if em6:
                            txt = "MFCCs\n" + get_title(pred, CAT6)
                            print(CAT6[pred.argmax()]) #Imprime emocion en pantalla
